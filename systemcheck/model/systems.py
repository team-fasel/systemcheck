...
#
#     system_tree_id = Column(Integer, ForeignKey('system_tree.id'), qt_show=False)
#     tree = relationship('SystemTreeNode', back_populates='abap_client', cascade='all, delete-orphan')
...
class AbapSystem(Base, QtModelMixin):
    ...
    tree = relationship('SystemTreeNode', back_populates='abap_system', cascade='all, delete-orphan', single_parent=True)
...
@generic_repr
class SystemTreeNode(Base, QtModelMixin):
...
#    abap_client = relationship('AbapClient', uselist=False, back_populates='tree')
